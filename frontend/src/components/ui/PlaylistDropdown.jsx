    ...
    <div className="relative w-full" ref={dropdownRef}>
      <div className="relative flex items-center">
        <input
          ...
          className={`peer flex h-12 w-full px-4 py-2 pr-10 text-sm xl:text-base outline-none`}
        />
      </div>
      ...
      <AnimatePresence>
        {open && history.length > 0 && (
          ...
          >
            <div
              className={`p-2 border-b text-xs font-semibold uppercase tracking-wider flex items-center gap-1 ${themeConfig.dropdownHeader}`}
            >
            ...
            <ul className="max-h-60 overflow-auto">
              {history.map((item) => (
                <li key={item.id}>
                  ...
                  >
                    <span className={`font-medium truncate w-full block ${themeConfig.dropdownTextMain}`}>
                      {item.title}
                    </span>
